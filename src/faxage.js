...
/**
 * Send a fax via the Faxage HTTPS API. Ported from the proven MOF backend
 * sender (mof-emr-backend/src/utils/common.js sendFax): multipart POST with
 * operation=sendfax and a base64-encoded PDF in faxfiledata[0]; success is
 * signalled by a "JOBID: <n>" token in the response body.
 */
async function sendFax({ faxTo, pdfBase64, fileName, recipName, tagName, tagNumber }, cfg) {
  ...
  if (res.status !== 200) {
    return { ok: false, jobId: null, raw: `HTTP_${res.status}: ${raw.slice(0, 300)}` };
  }
  const match = raw.match(/JOBID:\s*(\d+)/i);
  if (match) return { ok: true, jobId: match[1], raw };
  return { ok: false, jobId: null, raw: raw.slice(0, 300) };
}
